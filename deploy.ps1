...
function Get-ComposeCommand {
    try {
        if ((docker compose version 2>&1) -match "Docker Compose") {
    ...
    Write-Error "Neither 'docker compose' nor 'docker-compose' command found. Please install Docker."
    exit 1
}
...
Write-Info "Using command: $composeCmd"
...
try {
    Push-Location $projectDir
    & ([scriptblock]::Create("$composeCmd down --remove-orphans 2>&1")) | Out-Null
...
$output = & ([scriptblock]::Create("$composeCmd up --build -d 2>&1"))
...
while ($attempt -lt $maxAttempts -and -not $healthy) {
    try {
        $response = Invoke-WebRequest -Uri "http://localhost:5109/health" -Method Get -ErrorAction SilentlyContinue -TimeoutSec 2 -UseBasicParsing
    ...
    if (-not $healthy) {
        ...
        if ($attempt -lt $maxAttempts) {
            ...
            Write-Error "Check logs with: $composeCmd logs app"
            exit 1
        }
    }
}
...
if (-not $SkipMigrations) {
    ...
    try {
        & ([scriptblock]::Create("$composeCmd -f `"$dockerComposeFile`" exec -T app dotnet ef database update 2>&1")) | Out-Null
...
& ([scriptblock]::Create("$composeCmd -f `"$dockerComposeFile`" ps"))

Write-Info ""
Write-Success "Deployment completed successfully!"
Write-Info ""
Write-Info "Application is running at: http://localhost:5109"
Write-Info "To view logs: $composeCmd -f `"$dockerComposeFile`" logs -f app"
Write-Info "To stop containers: $composeCmd -f `"$dockerComposeFile`" down"
